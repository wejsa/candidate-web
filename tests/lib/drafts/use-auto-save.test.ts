...
// CANDID-039 Step 3 — 직접 구현한 minimal renderer(createRoot+act) → RTL renderHook 마이그.
//   RTL renderHook + fake timers 조합으로 동일 시나리오(debounce/zod가드/saveNow/409/5xx/
//   interval/cleanup/beforeunload)를 동작 동등하게 유지한다.
...
describe('useAutoSave — debounce', () => {
  it('notifyChange 후 debounceMs 경과 시 PUT 호출 (1회)', async () => {
    ...
    const { result, unmount } = renderUseAutoSave(fetcher, { debounceMs: 1000, intervalMs: 0 });
    act(() => result.current.notifyChange(makePayload()));
  ...
  it('연속 입력 시 debounce 재시작 → 마지막 입력 후 한 번만 PUT', async () => {
    ...
    act(() => result.current.notifyChange(makePayload(2)));
    await act(async () => {
...
describe('useAutoSave — zod 가드 (L-019 D-MAJOR-1)', () => {
  it('부분 입력 (만 14세 미만) → PUT 시도 안 함', async () => {
    const fetcher = makeFetcher(200, { version: 2, lastSavedAt: '2026-05-24T00:00:01.000Z' });
    const { result, unmount } = renderUseAutoSave(fetcher, { debounceMs: 100, intervalMs: 0 });
    ...
    act(() => result.current.notifyChange(invalidPayload));
    await act(async () => {
...
describe('useAutoSave — saveNow + saved 상태', () => {
  it('saveNow → 즉시 PUT + status="saved" + version 갱신', async () => {
    ...
    const { result, unmount } = renderUseAutoSave(fetcher, { debounceMs: 5000, intervalMs: 0 });
    act(() => result.current.notifyChange(makePayload()));
    await act(async () => {
...
describe('useAutoSave — 409 conflict 일시정지', () => {
  it('409 응답 → status="conflict" + 후속 자동 저장 차단', async () => {
    ...
    const { result, unmount } = renderUseAutoSave(fetcher as unknown as typeof fetch, {
      debounceMs: 100,
      intervalMs: 0,
    });
    act(() => result.current.notifyChange(makePayload()));
...
describe('useAutoSave — interval', () => {
  it('intervalMs 경과 + isDirty true → 자동 PUT', async () => {
    ...
    const { result, unmount } = renderUseAutoSave(fetcher, { debounceMs: 0, intervalMs: 1000 });
    act(() => result.current.notifyChange(makePayload()));
    await act(async () => {
      vi.advanceTimersByTime(1000);
      await Promise.resolve();
