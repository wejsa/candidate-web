...
| **환경 오류 escalation 트리거** | 가드 스크립트의 exit code 환경 오류(인프라 누락 등) 처리 정책 작성 시 "동일 PR이 환경 자체를 변경하는가?"를 escalation trigger로 사용. 환경 변경 PR(USER-LEVEL `scripts/check-*.mjs`, META-LEVEL `scripts/meta/check-*.mjs`, 또는 `package.json check:* script` 변경 포함)에서는 exit 2도 REQUEST_CHANGES로 격상해 *가드 무력화 + 위반 동시 제출* 공격 차단. META-LEVEL 가드는 `scripts/meta/`로 USER-LEVEL과 분리(CANDID-045 D-5)되므로 escalation 경로 매칭은 양쪽 디렉토리를 포함해야 한다. (L-039) | **MAJOR** |
| **Dynamic self-detection 권장** | self-validating tool(meta-guard 등 자기 자신을 참조해야 하는 도구)의 self-paths는 정적 하드코딩 상수 대신 `import.meta.url` 기반 `SELF_BASE`(basename 추출) + canonical cwd-relative 경로(`${dir}/check-${SELF_BASE}.{ext}`)로 동적 도출. rename 회귀 자동 추종 + fixture(tmpdir) 격리 호환. self-integrity check는 `existsSync(SELF_PATH)` gate로 production-only 실행. META-LEVEL 가드의 SELF 경로는 `scripts/meta/` 기준으로 도출. (L-042) | **MAJOR** |
| **가드 인프라 구조/보안 일관성** | 회귀 가드 인프라를 강화할 때: (a) runner 화이트리스트(`node`/`tsx`)는 self-detection 로직과 정합 유지 — 한쪽만 확장하면 self 미탐지로 self-DoS 오탐 발생 (b) orphan 스캔은 가드가 위치할 수 있는 *모든* 디렉토리(`scripts/` + `scripts/meta/`)를 커버 — 미스캔 디렉토리는 미배선 가드 은닉처(SSOT 우회) (c) lifecycle hook(`postinstall`/`prepublish` 등)이 가드를 직접 경로 또는 `npm/pnpm/yarn run check:*` alias로 암묵 실행하면 §2.4 SSOT 우회 — 차단. 단, wrapper 스크립트 경유 우회는 정적 분석 한계로 완전 차단 불가. (CANDID-045 D-3/D-4/D-5, M-SEC) | **MAJOR** |
| **가드 테스트 spawn/cleanup DRY** | 회귀 가드 테스트가 2개 이상이면 자식 프로세스 실행 + tmpdir fixture 정리 로직을 `tests/scripts/_helpers/spawn-guard.ts`(`runGuard(scriptPath, cwd)` + `createTmpTracker()`)로 공유한다. tracker는 파일별 독립 인스턴스를 생성해 테스트 파일 간 공유 가변 상태(워커 오염)를 만들지 않는다. (CANDID-045 Step 2) | **MINOR** |

> 본 항목은 CANDID-038/041/042/043/045 회고(L-034, L-035, L-036, L-038, L-039, L-042)에서 도입. closed loop **5-layer 방어 완성**: 가드 도입(038) → 가드 자체 테스트(041) → 가드 호출 wiring + 공급망 방어(042) → wiring 일관성 meta-guard + dynamic self-detection(043) → META-LEVEL 가드 디렉토리 분리(`scripts/meta/`) + runner 화이트리스트 + lifecycle-hook 우회 차단 + alias 의미 매칭 + 테스트 DRY(045).
