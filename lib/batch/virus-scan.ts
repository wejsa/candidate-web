...
// 주입된 scanner로 판정 → 상태 갱신. INFECTED는 S3 객체 삭제 + row 삭제 + owner 알림(BR-TX-02
// fire-and-forget). 외부 효과(스캔/S3/메일)는 모두 DI라 테스트가 결정적이고, tsx CLI에서도
// server-only 모듈을 거치지 않는다.
...
async function handleInfected(
  ...
  // owner 정보는 row 삭제 전에 선조회(email/name은 평문 컬럼).
  const owner = await db.user.findUnique({
    where: { id: file.ownerUserId },
    select: { email: true, name: true },
  });
  // S3 객체 삭제 — best-effort. 실패해도 row는 제거(메타 회수 우선; orphan S3는 후속 정리/lifecycle).
  try {
    await deps.deleteObject(file.storedPath);
  } catch {
    /* non-fatal */
  }
  await db.resumeFile.delete({ where: { id: file.id } });
  if (owner !== null) {
...
export async function processPendingScans(
  ...
  for (;;) {
    ...
    for (const file of files) {
      ...
      if (verdict === 'INFECTED') {
        await handleInfected(db, deps, file);
      } else {
        // CLEAN | FAILED
